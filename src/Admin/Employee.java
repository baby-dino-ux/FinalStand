 ...
 private final config conf = new config();
    ...
    private void setupComboBox() {
        reporttypecombobox.setModel(new javax.swing.DefaultComboBoxModel<>(new String[]{
            "All", "Available", "Busy", "Off Duty", "Pending", "Active"
        }));
        reporttypecombobox.addActionListener(e -> {
            String sel = (String) reporttypecombobox.getSelectedItem();
            filterEmployees(sel);
        });
    }
 
    private void filterEmployees(String filter) {
        String base = "SELECT id AS ID, firstname || ' ' || lastname AS Name, email AS Email, " +
                      "status AS Status, work_status AS 'Work Status' FROM tbl_users WHERE type='Employee'";
        switch (filter) {
            case "Available": conf.displayData(base + " AND work_status='Available' ORDER BY id DESC", TableCleaners); break;
            case "Busy":      conf.displayData(base + " AND work_status='Busy' ORDER BY id DESC", TableCleaners); break;
            case "Off Duty":  conf.displayData(base + " AND work_status='Off Duty' ORDER BY id DESC", TableCleaners); break;
            case "Pending":   conf.displayData(base + " AND status='Pending' ORDER BY id DESC", TableCleaners); break;
            case "Active":    conf.displayData(base + " AND status='Active' ORDER BY id DESC", TableCleaners); break;
            default:          loadEmployees("All"); break;
        }
    }
 
    // ── Load all employees ────────────────────────────────────────────────────
    private void loadEmployees(String ignored) {
        conf.displayData(
            "SELECT id AS ID, firstname || ' ' || lastname AS Name, email AS Email, " +
            "status AS Status, work_status AS 'Work Status' " +
            "FROM tbl_users WHERE type='Employee' ORDER BY id DESC",
            TableCleaners);
        loadEmployeeCounts();
    }
    ...
    private void loadEmployeeCounts() {
        availablenum.setText(count("work_status='Available'"));
        busynum.setText(count("work_status='Busy'"));
        offdutynum.setText(count("work_status='Off Duty'"));
        totalnum.setText(count(null));
    }
 
    private String count(String condition) {
        String sql = "SELECT COUNT(*) FROM tbl_users WHERE type='Employee'" +
                     (condition != null ? " AND " + condition : "");
        Object v = conf.getValue(sql);
            ...
            public void keyReleased(java.awt.event.KeyEvent e) {
                ...
                if (kw.isEmpty() || kw.equals("Search ")) {
                    ...
                    conf.displayData(
                        "SELECT id AS ID, firstname || ' ' || lastname AS Name, email AS Email, " +
                        "status AS Status, work_status AS 'Work Status' " +
                        "FROM tbl_users WHERE type='Employee' AND " +
                        "(firstname LIKE ? OR lastname LIKE ? OR email LIKE ?) ORDER BY id DESC",
                        TableCleaners, "%" + kw + "%", "%" + kw + "%", "%" + kw + "%");
                }
            }
        });
    }
 
    // ── Get selected row's user ID ────────────────────────────────────────────
    private int getSelectedId() {
        ...
        try { return Integer.parseInt(TableCleaners.getValueAt(row, 0).toString()); }
    ...
    private void initComponents() {
        ...
        refreshlistpanel.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                refreshlistpanelMouseClicked(evt);
            }
        });
        ...
        reporttypecombobox.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "All", "Available", "Busy", "Off Duty" }));
        reporttypecombobox.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                reporttypecomboboxMouseClicked(evt);
            }
        });
    ...
    private void blackMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_blackMouseClicked
         loadEmployees("All"); // clicking Total resets filter
    ...
    private void addpanelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_addpanelMouseClicked
        ...
        f.setVisible(true);
        // After form closes, refresh (using WindowListener)
        f.addWindowListener(new java.awt.event.WindowAdapter() {
            @Override public void windowClosed(java.awt.event.WindowEvent we) { loadEmployees("All"); }
        });
    }//GEN-LAST:event_addpanelMouseClicked

    private void updatepanelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_updatepanelMouseClicked
         int row = TableCleaners.getSelectedRow();
        if (row < 0) { JOptionPane.showMessageDialog(this, "Please select an employee.", "No Selection", JOptionPane.WARNING_MESSAGE); return; }
        String id        = TableCleaners.getValueAt(row, 0).toString();
        String fullName  = TableCleaners.getValueAt(row, 1).toString();
        String email     = TableCleaners.getValueAt(row, 2).toString();
        String status    = TableCleaners.getValueAt(row, 3).toString();
        String[] parts   = fullName.split(" ", 2);
        String fn = parts[0];
        String ln = parts.length > 1 ? parts[1] : "";
        Form f = new Form("Update", id, fn, ln, email, null, "Employee", status);
        f.setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
        f.setVisible(true);
        f.addWindowListener(new java.awt.event.WindowAdapter() {
            @Override public void windowClosed(java.awt.event.WindowEvent we) { loadEmployees("All"); }
    ...
    private void deletepanelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_deletepanelMouseClicked
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Delete this employee? This cannot be undone.", "Confirm Delete", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (confirm != JOptionPane.YES_OPTION) return;
        conf.deleteRecord("DELETE FROM tbl_users WHERE id = ?", id);
        JOptionPane.showMessageDialog(this, "Employee deleted.", "Done", JOptionPane.INFORMATION_MESSAGE);
        loadEmployees("All");
    ...
    private void approvepanelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_approvepanelMouseClicked
        ...
        if (row < 0) { JOptionPane.showMessageDialog(this, "Please select an employee.", "No Selection", JOptionPane.WARNING_MESSAGE); return; }
        String status = TableCleaners.getValueAt(row, 3).toString();
        if ("Active".equalsIgnoreCase(status)) { JOptionPane.showMessageDialog(this, "Employee is already Active.", "Already Active", JOptionPane.INFORMATION_MESSAGE); return; }
        int id = getSelectedId(); if (id < 0) return;
        conf.updateRecord("UPDATE tbl_users SET status='Active' WHERE id=?", id);
        JOptionPane.showMessageDialog(this, "Employee approved and set to Active.", "Approved", JOptionPane.INFORMATION_MESSAGE);
        loadEmployees("All");
    ...
    private void reporttypecomboboxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_reporttypecomboboxActionPerformed
     filterEmployees((String) reporttypecombobox.getSelectedItem());
    ...
    private void refreshlistpanelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_refreshlistpanelMouseClicked
           loadEmployees("All");
    }//GEN-LAST:event_refreshlistpanelMouseClicked

    private void reporttypecomboboxMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_reporttypecomboboxMouseClicked
        // TODO add your handling code here:
    }//GEN-LAST:event_reporttypecomboboxMouseClicked
