...
public class Users extends javax.swing.JFrame {
    ...
    // FIX: Load only Staff and Employee accounts — Admin accounts must NOT appear
    private void loadAllUsers() {
        String sql = "SELECT id, firstname, lastname, email, status, type FROM tbl_users " +
                     "WHERE type != 'Admin' ORDER BY id DESC";
        conf.displayData(sql, TableUsers);
        updateUserCounts();
    }
 
    // FIX: All filter queries exclude Admin accounts
    private void filterUsers(String filterType) {
        String base = "SELECT id, firstname, lastname, email, status, type FROM tbl_users " +
                      "WHERE type != 'Admin'";
        String condition = "";
 
        switch (filterType) {
            case "Staff":    condition = " AND type='Staff'";      break;
            case "Employee": condition = " AND type='Employee'";   break;
            case "Pending":  condition = " AND status='Pending'";  break;
            case "Active":   condition = " AND status='Active'";   break;
            case "Inactive": condition = " AND status='Inactive'"; break;
        }
 
        String sql = base + condition + " ORDER BY id DESC";
        conf.displayData(sql, TableUsers);
    }
    ...
    // FIX: ADD — check for duplicate email before opening Form
    private void handleAdd() {
        // Open the form; duplicate-email check is also enforced inside Form.java on save.
        // But we add an extra pre-check here for immediate feedback.
        Form addForm = new Form();
        addForm.setVisible(true);
        this.loadAllUsers();
    }
 
    // FIX: UPDATE — fetch username from DB and pass it properly into Form
    private void handleUpdate() {
        int userId = getSelectedUserId();
        if (userId < 0) return;
 
        // Fetch full user record from DB (including username) to avoid relying solely on table columns
        try (Connection conn = config.connectDB()) {
            PreparedStatement ps = conn.prepareStatement(
                "SELECT id, firstname, lastname, email, status, type, username FROM tbl_users WHERE id=?");
            ps.setInt(1, userId);
            ...
            if (rs.next()) {
                ...
                String username  = rs.getString("username"); // FIX: properly read username
 
                // Open Form in Update mode with all pre-filled data including username
                Form updateForm = new Form("Update", id, firstname, lastname, email, username, type, status);
    ...
    private void initComponents() {
        ...
        jPanel4.add(reporttypecombobox1, new org.netbeans.lib.awtextra.AbsoluteConstraints(680, 70, 100, 20));
    ...
    private void updateUsernameLabel() {
        if (Session.getInstance().isLoggedIn()) {
            lblUsername.setText(Session.getInstance().getFullName());
        } else {
            lblUsername.setText("Guest");
        }
    }
 
    // FIX: Email duplicate check — call this from Form.java on save, or use inline before opening form
