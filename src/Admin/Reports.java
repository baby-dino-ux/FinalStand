...
public class Reports extends javax.swing.JFrame {
    ...
    public Reports() {
        initComponents();
        setupReportComboBox(); // FIX: populate combobox with real options
        loadReportData();      // FIX: load all report data immediately on open
        loadTopCleaners();     // FIX: load top cleaners on open
    ...
    // FIX: Initialize the Report Type ComboBox with actual status options
    ...
    // FIX: Master method — loads dashboard panels, summary, and the report table
    private void loadReportData() {
        ...
        // Build a date-range condition (reusable)
        String dateCondition = "";
        if (!dateFrom.isEmpty()) dateCondition += " AND b_date >= '" + dateFrom + "'";
        if (!dateTo.isEmpty())   dateCondition += " AND b_date <= '" + dateTo + "'";
 
        try (Connection conn = config.connectDB()) {
 
            // ── Top dashboard panels ────────────────────────────────────────
 
            // Total Revenue — always from Done bookings, filtered by date
            PreparedStatement psRev = conn.prepareStatement(
                "SELECT COALESCE(SUM(b_price), 0) FROM tbl_bookings WHERE b_status='Done'" + dateCondition);
            ResultSet rsRev = psRev.executeQuery();
            if (rsRev.next()) totalrevenuenum.setText(String.format("%.2f", rsRev.getDouble(1)));
 
            // Total Bookings panel
            PreparedStatement psTotal = conn.prepareStatement(
                "SELECT COUNT(*) FROM tbl_bookings WHERE 1=1" + dateCondition);
            ResultSet rsTotal = psTotal.executeQuery();
            if (rsTotal.next()) totalbookingsnum.setText(rsTotal.getString(1));
 
            // Completed panel
            PreparedStatement psDone = conn.prepareStatement(
                "SELECT COUNT(*) FROM tbl_bookings WHERE b_status='Done'" + dateCondition);
            ResultSet rsDone = psDone.executeQuery();
            if (rsDone.next()) totalcompletednum.setText(rsDone.getString(1));
 
            // Pending panel
            PreparedStatement psPend = conn.prepareStatement(
                "SELECT COUNT(*) FROM tbl_bookings WHERE b_status='Pending'" + dateCondition);
            ResultSet rsPend = psPend.executeQuery();
            if (rsPend.next()) totalpendingnum.setText(rsPend.getString(1));
 
            // Cancelled panel
            PreparedStatement psCanc = conn.prepareStatement(
                "SELECT COUNT(*) FROM tbl_bookings WHERE b_status='Cancelled'" + dateCondition);
            ResultSet rsCanc = psCanc.executeQuery();
            if (rsCanc.next()) totalcancellednum.setText(rsCanc.getString(1));
 
            // ── Summary panel ───────────────────────────────────────────────
 
            summarytotalbookingsnum.setText(totalbookingsnum.getText());
            summarycompletednum.setText(totalcompletednum.getText());
            summarypendingnum.setText(totalpendingnum.getText());
            summarycancellednum.setText(totalcancellednum.getText());
 
            PreparedStatement psIP = conn.prepareStatement(
                "SELECT COUNT(*) FROM tbl_bookings WHERE b_status='In Progress'" + dateCondition);
            ResultSet rsIP = psIP.executeQuery();
            if (rsIP.next()) summaryinprogressnum.setText(rsIP.getString(1));
 
            summaryrevenuenum.setText(totalrevenuenum.getText());
        ...
        // ── Report table — filtered by type and date ────────────────────────
        String tableWhere = " WHERE 1=1" + dateCondition;
        if (!type.equals("All")) {
            tableWhere += " AND b_status='" + type + "'";
        }
 
        String tableSql = "SELECT b_id AS ID, b_customer AS Customer, b_service AS Service, " +
                          "b_date AS Date, b_status AS Status FROM tbl_bookings" +
                          tableWhere + " ORDER BY b_id DESC";
        conf.displayData(tableSql, TableRecentType);
    }
 
    // FIX: Load Top 5 Cleaners by completed task count, highest to lowest
    private void loadTopCleaners() {
        javax.swing.JLabel[] nameLabels = {firstname, secondname, thirdname, fourthname, fifthname};
        javax.swing.JLabel[] taskLabels = {firstnamenumoftask, secondnamenumoftask,
                                            thirdnamenumoftask, fourthnamenumoftask, fifthnamenumoftask};
 
        // Clear first
        for (int i = 0; i < 5; i++) {
            nameLabels[i].setText("-");
            taskLabels[i].setText("0");
        }
 
        String sql = "SELECT b_employee, COUNT(*) AS task_count " +
                     "FROM tbl_bookings WHERE b_status='Done' AND b_employee IS NOT NULL AND b_employee != '' " +
                     "GROUP BY b_employee ORDER BY task_count DESC LIMIT 5";
        ...
        try (Connection conn = config.connectDB()) {
            ...
            int i = 0;
            while (rs.next() && i < 5) {
                nameLabels[i].setText(rs.getString("b_employee"));
                taskLabels[i].setText(rs.getString("task_count"));
                i++;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void initComponents() {
        ...
        datefromfield.setForeground(new java.awt.Color(255, 255, 255));
        ...
        datetofield.setForeground(new java.awt.Color(255, 255, 255));
